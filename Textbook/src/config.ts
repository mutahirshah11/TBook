...
export const config = {
    // Auth Server URL
    // During build (SSR), we need an absolute URL. In browser, we use window.origin to construct absolute URL.
    authUrl: isDev 
        ? 'http://localhost:4000/api/auth'
        : (typeof window !== 'undefined' ? `${window.location.origin}/api/auth` : 'https://auth765.vercel.app/api/auth'),
